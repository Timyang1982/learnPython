...
class Settings(Common):
    ...
    def enter_settings(self, option):
        ...
        if self.device(text=self.appconfig("Settings","settings")).wait.exists(timeout = 2000):
            ...
            if self.device(text=option).exists:
                self.logger.debug("enter %s"%option)
    ...
    def switch_network(self,type = None):
        ...
        if self.enter_settings(u"More…"):
            if self.device(text=self.appconfig("Settings","network_text")).exists:
                self.device(text=self.appconfig("Settings","network_text")).click()
            if self.device(text=self.appconfig("Settings","networkmode_text")).wait.exists(timeout=self.timeout):
                self.device(text=self.appconfig("Settings","networkmode_text")).click()
            if self.device(resourceId="android:id/text1").wait.exists(timeout=self.timeout):
    ...
    def _connect(self,hotspot,password,security):
        ...
        if self.device(resourceId="com.android.settings:id/ssid").wait.exists(timeout=self.timeout):
            ...
            if password != "":
                ...
                self.device.delay(1)
                if self.device(text=security).exists:
                    self.logger.debug("security exists.")
                    self.device(text=security).click()
                else:
                    self.logger.warning("security not exists")
                self.device.delay(2)

                if self.device(resourceId="com.android.settings:id/password").exists:
                    self.logger.debug("input password")
                    self.device(resourceId="com.android.settings:id/password").set_text(password)
                else:
                    self.logger.warning("com.android.settings:id/password not exists.")
                self.device.delay(2)

        if self.device(text="Save").exists:
            self.logger.debug("click save")
            self.device(text="Save").click()
        else:
            self.logger.warning("Save not exists")

        self.device.delay(2)
        #self.device(scrollable=True).scroll.vert.toBeginning(steps=10)
        if self.device(text="Connected").wait.exists(timeout=20000):
